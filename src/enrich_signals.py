...
NEW_COLUMNS_SQL = {
    ...
    "insider_top_sellers": "VARCHAR",  # JSON list serializado
...
def fetch_fundamentals(ticker: str) -> dict:
    """
    Busca fundamentals básicos via yfinance.info.

    Retorna dict com:
    - pe_ttm: P/E últimos 12 meses
    - pe_forward: P/E estimado próximos 12 meses
    - eps_ttm: EPS últimos 12 meses
    - eps_forward: EPS estimado
    - dividend_yield: % anual
    ...
    - revenue_growth_yoy: crescimento de receita YoY
    - earnings_growth_yoy: crescimento de lucro YoY
    - next_earnings_date: próxima data de earnings (ISO)
    - short_percent_of_float: % do float em short
    ...
    try:
        ...
        # Mapeia campos do yfinance pros nossos nomes
        field_map = {
            "trailingPE": "pe_ttm",
        ...
        # Próxima earnings — yfinance .calendar é Calendar object ou dict
        try:
            cal = t.calendar
            if cal is not None:
                if isinstance(cal, dict):
                    ...
                    if earnings_dates and len(earnings_dates) > 0:
                        ...
                        if hasattr(d, "isoformat"):
                            result["next_earnings_date"] = d.isoformat()
                        else:
